...
\begin{table}[htb]
    ...
    \begin{tabular}{ccccccc}
        ...
        Entry & Algorithm & $J_\text{min}$ & $J_\text{max}$ & $f_\text{sos} / 10^7$ & FEs & Time taken (\unit{\s}) \\
        \midrule
...
\begin{figure}[htb]
    ...
    \caption[HMBC spectra before and after optimisation]{
        \textbf{(\subref{fig:poise_hmbc_spec_unopt2})} HMBC spectrum with `compromise' values of 120 and \qty{180}{\Hz} for $J_\text{min}$ and $J_\text{max}$ respectively in the second-order LPJF.
...
\begin{table}[htb]
    ...
    \begin{tabular}{ccccccc}
        \toprule
              &           & \multicolumn{3}{c}{Best optimum found} & \multicolumn{2}{c}{Aggregated results} \\
        \cmidrule(lr){3-5} \cmidrule(lr){6-7}
        Entry & Algorithm & $J_\text{min}$ & $J_\text{max}$ & $f_\text{sos} / 10^9$ & FEs & Time taken (\unit{\s}) \\
        \midrule
...
\begin{figure}[htb]
    ...
    \caption[NOAH HMBC spectra before and after optimisation]{
        ...
        \textbf{(\subref{fig:poise_hmbc_noah_spec_opt})} NOAH HMBC spectrum after POISE `optimisation' (using $J_\text{min} = \qty{125.2}{\Hz}$, $J_\text{max} = \qty{193.2}{\Hz}$, as per entry 2 of \cref{tbl:poise_hmbc_noah}).

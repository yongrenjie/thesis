...
Finally, in order to reduce the time taken for the optimisation, several tricks are used: each FE is run using no dummy scans and only one scan, the acquisition time is set to just \SI{1.1}{\s} (high resolution is not required for a reliable cost function value), and the relaxation delay \texttt{D1} is set to 0.
In practice, there is an extra gap of $\sim$ \SI{5}{\s} between successive FEs due to spectrometer initialisation, so an extra relaxation delay is not needed.
...
\Cref{tbl:poisecal_43,tbl:poisecal_53} summarise the results obtained with an initial guess of \SI{43}{\us} and \SI{53}{\us} respectively.

...
For a pulse--acquire experiment (\cref{fig:zg_ernst}), the repetition time is the sum of the acquisition time \texttt{AQ} plus the relaxation delay \texttt{D1}; the flip angle $\theta$ is controlled via the pulse width \texttt{P1}.
...
\begin{figure}[htb]
    \centering
    \includegraphics[draft=false]{poise/ernst_sensitivity.png}
...
To determine the accuracy of the optima found, I measured $T_1$ of each of these peaks using a typical gradient-enhanced inversion--recovery experiment, and calculated the theoretical Ernst angles from this (\cref{tbl:ernst_invrec}).
...
This could then be used to inform the choice of relaxation delay for multidimensional experiments\autocite{Reynolds2002JNP,Burns2021MRC} or quantitative NMR experiments\autocite{Pauli2005JNP,Giraudeau2014MRC}.

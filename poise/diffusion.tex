...
\begin{figure}[htb]
    \centering
    \includegraphics[draft=false]{pp/poise/dosy.png}
...
\begin{figure}[htb]
    \centering
    \includegraphics[draft=false]{poise/diffusion_sim.png}%
...
\begin{figure}[htb]
    \centering
    \includegraphics[draft=false]{poise/dosy_flowchart.png}%
...
Thus, the wrapper script should always be started with the smallest value of $\Delta$ which is sensible for the sample under study: in the present case, I used an initial value of \SI{50}{\ms}.
...
Thus, $\Delta$ is increased by a fixed amount (\SI{50}{\ms} in this case), and the sub-optimisation step repeated.\footnote{If sample spinning is used, then this incrementation of $\Delta$ needs to be chosen carefully, as the diffusion delay should always correspond to an integral number of rotations.}
...
As described previously, the procedure was initialised with a value of $\Delta = \SI{50}{\ms}$.
The gradient duration $\delta$, which is not modified, was set to \SI{2}{\ms}.
Invariably, the first sub-optimisation increased this diffusion delay to \SI{100}{\ms}.
Since this part only uses one function evaluation in POISE, the optimisation algorithm used does not actually matter.
However, the algorithm does matter for the second sub-optimisation (which searches for the `ideal' $G_\text{max}$).
...
\begin{table}[htb]
    ...
    \end{tabular}
    \caption{
        Results of gradient amplitude sub-optimisation for diffusion NMR spectroscopy.
...
The full Oneshot DOSY spectrum was then acquired using these parameters of $\Delta = \SI{100}{\ms}$, $\delta = \SI{2}{\ms}$, and $G_\text{max} = 75\%$ (nominally \SI{49.3}{\gauss\per\cm} on the spectrometer used here, see \cref{tbl:spectrometers}).
...
This figure may of course be larger depending on the number of scans used (here I used \texttt{DS=2} and \texttt{NS=4}), as well as how many iterations there are in the first sub-optimisation (each iteration contributes two FEs since the reference spectrum with $G_\text{min}$ must be re-acquired when $\Delta$ is changed).
...
(The equation was obtained from the pulse sequence provided on the University of Manchester website; here $\tau$ represents the time between the midpoints of each gradient within the bipolar pulse pair, and $\alpha$ the gradient imbalance factor as shown in \cref{fig:poise_dosy_pulseq_oneshot}.)

\begin{figure}[htb]
    \centering
    \includegraphics[draft=false]{poise/dosy_profiles.png}%
...
\subsubsection{Simultaneous optimisation of $\Delta$ and $G_\text{max}$}
...
In order to ensure that $\Delta$ is not increased by too much, we must add a penalty term to the cost function:
\begin{equation}
    \label{eq:dosy_2p_cf}
    f_\text{dosy,2p} = f_\text{dosy} + (\Delta / \unit{\s})
\end{equation}

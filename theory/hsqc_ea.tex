...
Recall that the Hamiltonian caused by a gradient of strength $G$ is given by $\Hgrad = \sum_i \gamma_i Gz I_{iz}$ (\cref{eq:h_grad}), where $z$ is the $z$-position of the spin.
...
So, we can directly apply $\exp(-\mi\Hgrad\tau)$ to the density operator in \cref{eq:hsqc_ea_before_g2} to get:%
...
On the other hand, the first and third terms are attenuated by a factor proportional to $\int_{-L/2}^{L/2} \cos[(\gamma_S g_1 + \gamma_I g_2)z\tau]\,\md z$ (or the equivalent sine-modulated integrand).

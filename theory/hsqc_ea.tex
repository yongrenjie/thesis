...
\begin{figure}[htbp]
    ...
    \caption[Experimental comparison of States--TPPI and echo--antiecho HSQC]{
        ...
        \textbf{(\subref{fig:hsqc_comparison_ea_ns1})} Echo--antiecho HSQC with one scan.
        ...
        \textbf{(\subref{fig:hsqc_comparison_projections})} Projections of the States--TPPI and echo--antiecho spectra in (\subref{fig:hsqc_comparison_st_ns1}) and (\subref{fig:hsqc_comparison_ea_ns1}) onto the $F_2$ axis (to avoid picking up the artefacts, only the region between 60 and \SI{110}{ppm} in $F_1$ was projected).
        The two projections are slightly horizontally offset for visual clarity.
        The signal intensity is the same, but the noise level in the echo--antiecho spectrum is larger (by a factor of $\sqrt{2}$, as described in the text).
        \datacode{6A-220809}
    }
...
Comparing the projections of this and the echo--antiecho experiment (\cref{fig:hsqc_comparison_projections}), it can be seen that the States--TPPI version has a larger SNR by a factor of $\sqrt{2}$; however, it clearly requires at least a two-step phase cycle to obtain presentable data, as was done in \cref{fig:hsqc_comparison_st_ns2}.
Clearly, the sensitivity of the echo--antiecho spectrum in \cref{fig:hsqc_comparison_ea_ns1} is good enough, rendering the extra SNR from the States--TPPI method unnecessary: it is therefore more practical to use the echo--antiecho version, as the spectrum quality is much better.

...
\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.8]{prodops_rules.png}
...
As per \cref{fig:prodops_rules}, this transforms the $-I_y$ term to $-2I_xS_z$ at point \circled{2}: the Hamiltonian is $\pi J_{IS} 2I_zS_z$ for a total time of $2\Delta = 1/(2J_{IS})$, so the `angle' rotated through is $\pi J_{IS}/(2J_{IS}) = \pi/2$.
...
However, the heuristics such as those in \cref{fig:prodops_rules} \textit{are} limiting, in that the evolution under some Hamiltonians---for example, strong coupling $\symbf{I}\cdot\symbf{S}$, or pulses for off-resonance spins where $H$ is a sum of $I_x$ and $I_z$---cannot be neatly captured in such a pictorial form.

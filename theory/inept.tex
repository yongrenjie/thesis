...
\begin{figure}[htbp]
    ...
    \caption[Simplified rules for product operator evolutions]{
        ...
        For example, a \angang{90}{x} pulse has the `base' operator $I_x$ and the angle $\omega\tau = \pi/2$; thus, the initial state $I_z$ would be rotated to $I_z\cos(\pi/2) - I_y\sin(\pi/2) = -I_y$.
    }
...
In practice, instead of subtracting the two signals, it is typical to shift the receiver phase $\phirec$ by \ang{180} in the second experiment: this introduces a phase shift of $\exp(-\mi\pi) = -1$ to the signal, and the two signals can now be \textit{added} together instead of subtracted to cancel out $s_S$.
Since both $\phi_1$ and $\phirec$ are $0$ on the first experiment and $\pi$ on the second experiment, we can express this as $\phi_x = \phirec = (0, \pi)$.
This is more commonly denoted as $\phi_1 = \phirec = (x, -x)$, because the phases $(0, \pi)$ correspond to the $+x$- and $-x$-axes respectively.

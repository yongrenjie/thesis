...
In \cref{subsec:pureshift__elements}, the time-reversal element was briefly introduced; I now provide a more theoretical description of it.
As stated in that section, this merely consists of a hard pulse with a flip angle $\beta$, and using \cref{eq:ipm_beta_evolution,eq:ialpha_beta_evolution,eq:ibeta_beta_evolution} we first analyse its potential use as a JRE.
...
\begin{figure}[htb]
    \centering
    \includegraphics[draft=false]{pp/pureshift/timereversal.png}
...
As before, for a system containing $N$ mutually coupled spins, $N$ different experiments must be acquired:
\begin{align}
...
Note that the weights for the PSE correspond to that used for the ECOSY experiment, and the weights for the JRE correspond to that used for the complementary ECOSY experiment.

In practice, $N = 5$ is likely to cover most realistic spin systems: this means that we have $\beta = \{\ang{36}, \ang{72}, \ang{108}, \ang{144}, \ang{180}\}$ and $W = \{−6.545,1.809,−0.955,0.691,−0.625\}$.
\Cref{fig:timereversal_insets} shows insets of the five subspectra acquired with the above values of $\beta$ and scaled by their respective weights.
All of the spectra have been phased using the automatic phase corrections determined for the weighted sum (i.e.\ the pure shift spectrum).

\begin{figure}[htb]
    \centering
    \includegraphics[draft=false]{pureshift/timereversal_insets.png}
...
Although the experiment seems to work, in that the sum \textit{is} indeed a pure shift spectrum, the fact that it is obtained through summation of $N$ different experiments brings some immediate drawbacks.
...
As a result, random distortions are observed around the desired peaks in the pure shift spectrum: this is noticeable in the \SI{4.04}{ppm} peak in \cref{fig:timereversal_insets}, and is even worse for intense signals.
...
The idea of optimising the weights to better suppress artefacts was also briefly considered: however, given that \cref{eq:timereversal_pse_weights} already yields \textit{theoretically} complete suppression, it was deemed unlikely that anything substantially better could be obtained.

\begin{figure}[htb]
    \centering
    \includegraphics[draft=false]{pureshift/timereversal_sensitivity.png}
...
This suggests that the `TSE' time-reversal experiment in \cref{fig:timereversal_pulseq} is a viable pure shift sequence without requiring the summation of multiple subspectra---albeit one which does not have any significant advantage over PSYCHE.

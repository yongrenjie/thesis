...
\begin{align}
    \phi(t) &= \phi_0 + 2\cpi f_0 t + \cpi\taup(\Delta F)\left(\frac{t}{\taup} - \frac{1}{2}\right)^2 \label{eq:chirp_pulse_phase} \\
...
\begin{figure}[htb]
    ...
    \caption[Phase and Cartesian amplitudes of a typical chirp pulse]{
        ...
        \textbf{(\subref*{fig:chirp_coefficients_phase})} $\phi(t)$ wrapped to the range $[0, 2\cpi)$.
        \textbf{(\subref*{fig:chirp_coefficients_smoothing})} The quarter-sine smoothing profile which is later applied to the entire waveform (\cref{eq:sming_function}); here $s_{\symup{sm}} = 0.2$.
...
The entire waveform was then multiplied by a \textit{smoothing function} $f_{\symup{sm}}(t)$ (\cref{fig:chirp_coefficients_smoothing}), which prevents large jumps in RF amplitude at the beginning and end of the pulse.
$f_{\symup{sm}}$ depends on a smoothing parameter $s_{\symup{sm}}$, which is typically $0.1$--$0.2$:
\begin{equation}
    \label{eq:sming_function}
    f_{\symup{sm}}(t) = \begin{cases}
        \displaystyle \sin\left(\frac{\cpi t'}{2 s_{\symup{sm}}}\right) & 0 \leq t' < s_{\symup{sm}}; \\
        \displaystyle 1 & s_{\symup{sm}} \leq t' < 1 - s_{\symup{sm}}; \\
        \displaystyle \sin\left[\frac{\cpi (1 - t')}{2 s_{\symup{sm}}}\right] & s_{\symup{sm}} \leq t' \leq 1, \\
    \end{cases}
\end{equation}
...
\begin{figure}[htbp]
    ...
    \caption[Spurious optimum obtained in waveform optimisation using $\fdiff$]{
        ...
        The grey dotted box shows the restricted region over which $\fdiff$ was subsequently applied to, yielding a formally different cost function $\fdiff'$; this yielded more sensible results where the cost function for the saltire pulse was smaller.
        \datacode{6C-190823}
    }
    \label{fig:chirpopt_spurious}
\end{figure}

Using this setup, several optimisations of the 12 parameters above were conducted using the $\fdiff$ cost function, which had performed well enough in the flip angle optimisations.
...
Although the initial point (a saltire) yielded a much larger SNR (\cref{fig:chirpopt_spurious_saltire}), the value of $\fdiff$ was still larger (i.e.\ worse) than for this false optimum.
...
This yields a formally different cost function, which I label as $\fdiff'$.
...
This is a meaningful result, as it demonstrates that $\fdiff'$ is actually an accurate metric to determine the quality of a PSE (it is noisy, but this is to be expected of an experimentally measured cost function).

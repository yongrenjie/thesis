...
\begin{figure}[htb]
    ...
    \caption[EPSI PSYCHE-iDOSY and associated pulse sequences]{
        ...
        \textbf{(\subref*{fig:epd_pulseq_psychedosy})} PSYCHE-iDOSY experiment. $\tau$ is set to $1/(4 \cdot \Tchunk)$, as before.
...
However, the detection step is accomplished using the \ac{epsi} acquisition technique\autocite{Mansfield1977JPCSSP,Stehling1991S}, much like in ultrafast 2D NMR (\cref{fig:epd_pulseq_epsidosy})\autociteset{ultrafast}.
...
The sequence itself was written and evaluated by Corentin Jacquemmoz and \JND{} at the University of Nantes, using a concentrated (\qty{1}{\molar}) sample of ethanol in \ch{D2O}.

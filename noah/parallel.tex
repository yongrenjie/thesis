...
\footnote{This description is slightly simplified: it omits the fact that all three modules must be acquired twice per $t_1$ increment for echo--antiecho or States--TPPI quadrature detection to be carried out. This is mostly an implementation detail, however; it does not materially affect the concepts described in this section.}
...
This generally has very little impact on the spectra (as was previously shown), but conveniently leads us into the final example of \cref{fig:parallel_noah_overview_parallel}.
Here, instead of acquiring each increment of the HSQC module twice, this slot is used to acquire two different modules, an in-phase (IP) and antiphase (AP) HSQC (both run without \carbon{} decoupling).
...
Likewise, the $F_2$-coupled seHSQC experiment is also performed in an IPAP manner, where this time the IPAP components are the two peaks in each doublet (split by $\oneJ{CH}$).
...
\begin{figure}[!ht]
    ...
    \caption[IPAP processed spectra from NOAH-8 supersequence]{
        ...
        \textbf{(\subref*{fig:tsnoah_ipap_spec_s_alpha})--(\subref*{fig:tsnoah_ipap_spec_s_beta})} IPAP-processed seHSQC spectra (from \cref{fig:tsnoah_example_s1,fig:tsnoah_example_s2}), each of which contain one peak from every \proton{}--\carbon{} doublet.
    }
...
In TS NMR, pulse sequences are specially designed in order to record two (or more) different signals at once, such as a \carbon{} HSQC and a \nitrogen{} HSQC: this has found use in both biomolecular\autocite{Farmer1991JMR,Boelens1994JBNMR,Pascal1994JMRSB,Sattler1995JBNMR,Frueh2009JBNMR,Lohr2014JMR} and small-molecule NMR\autocite{Nolis2006MRC,PerezTrujillo2007OL,Nolis2007ACIE,Parella2010CMR,Kupce2021NRMP}.
...
However, if these signals need to be separated, then the TS experiment must be repeated: once where the two signals have the same phase, and once where they have opposite phases.
...
Equivalently, the time savings (or sensitivity improvement) from a TS experiment stem from the fact that it is being compared against a pair of `base experiments' which each yield only one of the two signals detected in the TS sequence.
To justly label an experiment as being TS, appropriate choices must be made for these `base experiments'.
...
For example, a supersequence with $m_1 = 1$ and $m_2 = 2$ is shown in \cref{fig:generalised_example}.
Generally, it is desirable to assign larger $m_i$ values to less sensitive modules, as the FIDs can be summed up to yield improved signal-to-noise.

\begin{figure}[!ht]
    \centering
    \includegraphics[]{noah/generalised_example.png}%
    \caption[Basic example of a generalised supersequence]{
        ...
        For each value of $t_1$, the TOCSY is acquired once, the NOESY twice, and the ($k$-scaled) HSQC three times, as indicated by the subscripts in the condensed representation on the right.
    }
...
Thus, the pulse programmes for this final section have been written by hand.
...
The values of $m_1$ and $m_2$ are encoded as \texttt{CNST52} and \texttt{CNST53}; the latter of these can be automatically calculated, since $m_2 = M - m_1$.
...
(It does not need to know the \textit{values} of the $m_i$'s, since that is a parameter to be set by the user.)
...
However, the \textit{phase cycle} is only moved forward within the inner \texttt{NS} loop; each time \texttt{NS} repetitions are completed, the phase cycle is reset to the beginning.
...
\begin{figure}[!ht]
    ...
    \caption[Examples of generalised supersequences]{
        Two examples of generalised NOAH supersequences studied in this work.
...
In \cref{fig:covariance_spec_bruc_cn}, the brucine \nitrogen{} HMBC and \carbon{} HSQC spectra  (\cref{fig:abbs_spec_bn,fig:abbs_spec_s}) are processed using unsymmetrical indirect covariance (\cref{eq:covariance_unsymmetrical}) to yield a \carbon{}--\nitrogen{} correlation spectrum which contains both one-bond and long-range correlations.\autocite{Martin2007JHC,Martin2007MRC}
The sign of the peaks indicates the multiplicity of the \carbon{} nucleus involved, and stems from the use of multiplicity editing in the \carbon{} HSQC spectrum.
...
This does not pose a problem in the case of cyclosporin, though, as the only quaternary carbons are those of the carbonyl (\ch{CO}) groups, which will still display correlations to the adjacent \ch{C}$\upalpha$ carbons.
...
\Cref{fig:covariance_spec_cyclo_cacb_nosymm,fig:covariance_spec_cyclo_cacb_symm} show the \ch{C}$\upalpha$--\ch{C}$\upbeta$ correlations.
...
It should be emphasised that such a procedure can only be safely carried out where peaks on both sides of the diagonals are expected to be seen.%
\footnote{The peak intensities $p(\Omega_1, \Omega_2)$ and $p(\Omega_2, \Omega_1)$ need not be exactly equal, but both peaks should at least be above the noise level.}
Thus, if the symmetrisation procedure were to be applied across the entire spectrum, correlations between quaternary and non-quaternary carbons (such as those in \cref{fig:covariance_spec_cyclo_caco}) would be lost.
However, since all \ch{C}$\upalpha$ and \ch{C}$\upbeta$ carbons in cyclosporin are protonated, the symmetrisation process is safe to use in this region; in this case, it helps to remove some of the artefacts which tend to plague the interpretation of covariance spectra.

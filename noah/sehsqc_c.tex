...
\begin{figure}[!htbp]
    ...
    \caption[CRK seHSQC and NOAH seHSQC modules]{
        ...
        This analysis is provided only for the first step of the phase cycle, and assumes a \ch{CH} spin pair with $\Delta' = 1 / (4 \cdot \oneJ{CH})$.
...
It should, however, be noted that the PEP transfer is only fully realised for $IS\/$ spin pairs (i.e.\ \ch{CH} groups) if the delay $\Delta'$ is set to $1 / (4 \cdot \oneJ{CH})$.
For $I_2S\/$ or $I_3\/S$ spin systems (i.e.\ \ch{CH2} or \ch{CH3} groups), no gain in sensitivity is accomplished with this setting of $\Delta' = 1 / (4 \cdot \oneJ{CH})$.
It is more common, therefore, to shorten $\Delta'$ to $1 / (8 \cdot \oneJ{CH})$: this sacrifices some transfer efficiency for $\ch{CH}$ groups, but allows for some sensitivity enhancement for \ch{CH2} and \ch{CH3} groups (\cref{tbl:sehsqc_theory}).
...
\begin{table}[!ht]
    \begin{tabular}{cccc}
        ...
        \cmidrule(lr){2-4}
                             & $\Delta' = 1 / (4 \cdot J_{IS})$ & $\Delta' = 1 / (8 \cdot J_{IS})$ & $\Delta' = 1 / (12 \cdot J_{IS})$ \\
        \midrule
        $IS$   & 2 & 1.71 & 1.5  \\
        $I_2S$ & 1 & 1.41 & 1.37 \\
        $I_3S$ & 1 & 1.21 & 1.25 \\
...
In particular, the CTP gradients in the CRK seHSQC dephase the bulk \magnnot{C} magnetisation pool: we would very much like this to be instead returned to $+z$, so that it can be sampled in later homonuclear modules (or, indeed, an HMBC module).
...
At the beginning of the final spin echo, this is transformed into antiphase magnetisation of the form $2I_xS_z$; thus, this spin echo must be lengthened to a total duration of $2\Delta = 1/(2 \cdot \oneJ{CH})$ in order to fully refocus $\oneJ{CH}$.
...
Instead of this, the original paper had inserted a purge gradient between the \proton{} and \carbon{} \ang{90} pulses just after the INEPT spin echo (\cref{fig:sehsqc_grad_schemes}).
...
Naturally, the primary consideration when evaluating the different seHSQC versions in \cref{fig:sehsqc_po} is sensitivity.
...
In the CRK seHSQC, these arise due to evolution of $\nJ{HH}$ during the second-last spin echo.
...
This leads to a peak correlating spin $S$ with spin $K$.
...
The COSY-type artefacts, labelled with red boxes in the seHSQC2 spectrum (\cref{fig:sehsqc_cosy_arts_2}), are largely attenuated in the seHSQC1 spectrum (\cref{fig:sehsqc_cosy_arts_1}).
...
\begin{figure}[!ht]
    ...
    \caption[Comparison of COSY-type artefacts in NOAH seHSQC modules]{
        ...
        \textbf{(\subref*{fig:sehsqc_cosy_arts_1})} \mbox{seHSQC1}.
...
\begin{figure}[!ht]
    ...
    \caption[More detail about wing artefacts in CLIP-COSY spectra]{
        ...
        \textbf{(\subref*{fig:sehsqc_wing_arts_proj_offon})} With only the gradient after $t_1$ (the intensity of the gradient before $t_1$ is set to 0).
        \textbf{(\subref*{fig:sehsqc_wing_arts_proj_onoff})} With only the gradient before $t_1$.
        \textbf{(\subref*{fig:sehsqc_wing_arts_proj_offoff})} With neither gradient.
        \textbf{(\subref*{fig:sehsqc_wing_arts_proj_offon_sw4})} With only the gradient after $t_1$, and additionally with the seHSQC indirect-dimension spectral width reduced by a factor of 4.
        \datacode{7A-201115}
    }
...
Fortunately, it proves to be relatively easy to suppress them through judicious use of gradients.
...
\begin{figure}[!ht]
    ...
    \caption[Multiplicity-edited NOAH seHSQC modules]{
        ...
        All symbols have the same meaning as in \cref{fig:sehsqc_po}.
    }
...
\begin{figure}[!ht]
    ...
    \caption[Sensitivity comparisons for multiplicity-edited seHSQC]{
        Sensitivity comparisons for \noah{Sp,Cc} supersequences with multiplicity editing included; the reference intensities are taken from a likewise edited \noah{S,Cc} supersequence.
...
As discussed previously, there are several possible values for the delay $\Delta'$ in the seHSQC experiment.
I therefore also specifically investigated the possibility of setting $\Delta' = 1 / (4 \cdot \oneJ{CH})$; the corresponding sensitivity comparisons are shown in \cref{fig:noah_sehsqc_1over4j}.
...
This reveals a geometric interpretation of this pulse element: it is actually a \ang{120} rotation about the vector $(1, 1, 1)$, which interchanges the three Cartesian axes; it is closely related to the $C_3$ symmetry operation in the octahedral point group ($O_{\symup{h}}$).}
...
The maximum $B_1$ allowed was \qty{15}{\kHz}; the optimisation was run with a 10\% variation in $B_1$ to account for the effects of RF inhomogeneity.
...
\begin{figure}[!ht]
    ...
    \caption[Simulated performance of URP used for seHSQC1 module]{
        Simulated performance of the optimised URP on $z$- and $y$-magnetisation. The pulse fidelity was 99.99\%.
...
The reason for the poorer sensitivity therefore likely lies elsewhere, although an actual explanation remains elusive.
